...
describe Nasa do
  ...
  context 'requesting information on a nasa browse works correctly' do

    before(:all) do
      @nasa = Nasa.new.nasa_neo_browse
      @nasa.get_neo_browse_data
    end
    ...
    it 'should return a hash for near earth objects' do
      expect(@nasa.get_near_earth_objects).to be_kind_of(Array)
    end
